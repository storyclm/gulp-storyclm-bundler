...
module.exports = (gulp, plugins, file_path, options, path) => (done) => {
    return gulp.src(options.project).pipe(gulpFn((file) => {
        ...
        fs.readFile(json_path, 'utf8', (err, data) => {
            ...
            console.log(`${options.project}../common/*.html`);
            for (let i = 0; i < seq_json.sequences.length; i++) {
                ...
                if (!fs.existsSync(`${options.project}/${seq_json.sequences[i]}/index.html`)) {
                    fs.copyFileSync(`${options.project}/${seq_json.sequences[i]}.html`,`${options.project}/${seq_json.sequences[i]}/index.html`);
