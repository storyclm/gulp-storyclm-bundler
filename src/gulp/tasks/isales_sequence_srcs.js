  ...
  return gulp.src(options.project).pipe(gulpFn((file) => {
    ...
    fs.readFile(json_path, 'utf8', (err, data) => {
      ...
      for (let i = 0; i < seq_json.sequences.length; i++) {
        ...
        search_src_links(seq_json.sequences[i], options.project);
      }
    })
  }))
  done();
};
function search_src_links(sequence, proj_f) {
  let slide = fs.readFileSync(`${proj_f}/${sequence}/${sequence}.html`, 'utf8'),
    common_folder = `${proj_f}/../common`,
  ...
  if (!fs.existsSync(`${proj_f}/${sequence}/css`)) {
    fs.mkdirSync(`${proj_f}/${sequence}/css`);
  }
  if (!fs.existsSync(`${proj_f}/${sequence}/js`)) {
    fs.mkdirSync(`${proj_f}/${sequence}/js`);
  }

  for (let c = 0; c < css_src__arr.length; c++) {
    fs.copyFileSync(`${common_folder}/${css_src__arr[c]}`, `${proj_f}/${sequence}/${css_src__arr[c]}`);
    let css_file = fs.readFileSync(`${proj_f}/${sequence}/${css_src__arr[c]}`, 'utf8');
    incss_links(css_file, `${proj_f}/${sequence}`);
  }
  for (let c = 0; c < js_src__arr.length; c++) {
    fs.copyFileSync(`${common_folder}/${js_src__arr[c]}`, `${proj_f}/${sequence}/${js_src__arr[c]}`);
  }
  function check_media_folder() {
    let result = '';
    if (!fs.existsSync(`${proj_f}/${sequence}/mediafiles`) && fs.existsSync(`${common_folder}/mediafiles`)) {
      fs.mkdirSync(`${proj_f}/${sequence}/mediafiles`);
      result = 'mediafiles';
    } else if (!fs.existsSync(`${proj_f}/${sequence}/media`) && fs.existsSync(`${common_folder}/media`)) {
      fs.mkdirSync(`${proj_f}/${sequence}/media`);
      result = 'media';
    } else if (fs.existsSync(`${proj_f}/${sequence}/mediafiles`)) {
      result = 'mediafiles';
    } else if (fs.existsSync(`${proj_f}/${sequence}/media`)) {
      result = 'media';
    }
    return result;
  }
  if (arr__src_mp3 != null) {
    let media_folder = check_media_folder();
    for (let m = 0; m < arr__src_mp3.length; m++) {
      fs.copyFileSync(`${common_folder}/${media_folder}${arr__src_mp3[m]}`, `${proj_f}/${sequence}/${media_folder}${arr__src_mp3[m]}`);
    }
  }
  if (arr__src_mp4 != null) {
    let media_folder = check_media_folder();
    for (let m = 0; m < arr__src_mp4.length; m++) {
      fs.copyFileSync(`${common_folder}/${media_folder}${arr__src_mp4[m]}`, `${proj_f}/${sequence}/${media_folder}${arr__src_mp4[m]}`);
    }
  }
  if (arr__src_pdf != null) {
    ...
    for (let m = 0; m < arr__src_pdf.length; m++) {
      ...
      fs.copyFileSync(`${common_folder}/${media_folder}/${pdf_file}`, `${proj_f}/${sequence}/${media_folder}/${pdf_file}`);
    }
  }
